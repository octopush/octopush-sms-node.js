...
const PATH_BALANCE = PATH + '/api/balance';
const PATH_SUB_ACCOUNT = PATH + '/api_sub/add_sub_account';
const PATH_CREDIT_SUB_ACCOUNT_TOKEN = PATH + '/api_sub/credit_sub_account_get_session';
const PATH_CREDIT_SUB_ACCOUNT = PATH + '/api_sub/credit_sub_account';
const PATH_OSTP = PATH + '/api/open_single_temp_session';
const PATH_EDIT_OPTIONS = PATH + '/api/edit_options';
const PATH_GET_USER_INFO = PATH + '/api_sub/get_user_info';
const _CUT_ = 7;
...
var OWS = (function(){
     ...
     * Credits sub account
     *
     * @param {string} sub_account_email - subaccount email
     * @param {int} sms_amount - number of credits
     ...
     */
    OWS.prototype.credit_sub_account = function(sub_account_email, sms_amount, sms_type, callback) {
        var self = this;
        var domain = DOMAIN;
        var path = PATH_CREDIT_SUB_ACCOUNT_TOKEN;
        var port = PORT;

        var data = {
            user_login: self.config.configuser_login,
            api_key: self.config.api_key,
            sub_account_email: sub_account_email
        };

        this._Request(domain, path, port, data, function(error, xml_return){
            if (error == true) {
                callback(true, xml_return);
            } else {
                var token = xml_return['octopush']['token'];
     ...
     * @param {function} callback - callback function that takes (error, response) parameters
     */
    OWS.prototype.edit_options = function(callback) {
        var domain = DOMAIN;
        var path = PATH_EDIT_OPTIONS;
        var port = PORT;
        ...
        if(this.config.answer_email != -1) {
            data['answer_email'] = this.config.answer_email;
        }
        if(this.config.sms_alert_bound != -1) {
            data['sms_alert_bound'] = this.config.sms_alert_bound;
        }
        if(this.config.sms_alert_type != -1) {
            data['sms_alert_type'] = this.config.sms_alert_type;
        }
     ...
     * @param {function} callback - callback function that takes (error, response) parameters
     */
    OWS.prototype.get_balance = function(callback) {
        var domain = DOMAIN;
        var path = PATH_BALANCE;
        var port = PORT;
        ...
        this._Request(domain, path, port, data, function(error, result){
            var balance = {};
            for(var i = 0; i < result.octopush.balance.length; i++) {
                balance[result.octopush.balance[i]['$'].type] = result.octopush.balance[i]['_'];
            }
            result.octopush.balance = balance;
            callback(error, result);
        });
    };
    ...
    OWS.prototype.set_sms_alert_type = function(sms_alert_type) {
        if([SMS_PREMIUM, SMS_STANDARD].indexOf(sms_alert_type) >= 0) {
...
var SMS = (function(){
    ...
    function SMS(user_login, api_key){
        this.config = {
            ...
            sms_fields_3: [],
            sms_mode: INSTANTANE,
            sending_date: Math.floor((new Date()).getTime() / 1000),
            sms_d: date("d"),
            sms_m: date("m"),
            sms_h: date("H"),
            sms_i: date("i"),
            sms_y: date("Y"),
            sms_sender: 'CampagneSMS',
            sms_type: SMS_STANDARD,
            request_mode: REEL,
            request_id: '',
            with_replies: 0,
            transactional: 0,
    ...
    SMS.prototype.send = function(callback){
        ...
        if(this.config.sms_mode == DIFFERE) {
            data['sending_date'] = this.config.sending_date;
        }
     ...
     * @param {int} alert_bound
     * @param {string} alert_sms_type - octopush.constants.SMS_STANDARD, octopush.constants.SMS_WORLD, octopush.constants.SMS_PREMIUM
     * @param {function} callback - callback function that takes (error, response) parameters
     */
    SMS.prototype.create_sub_account = function(first_name, last_name, raison_sociale, alert_bound, alert_sms_type, callback){
        var domain = DOMAIN;
        var path = PATH_SUB_ACCOUNT;
        var port = PORT;
     ...
     * @param {int} sms_amount
     * @param {string} sms_type - octopush.constants.SMS_STANDARD, octopush.constants.SMS_WORLD, octopush.constants.SMS_PREMIUM
     * @param {function} callback - callback function that takes (error, response) parameters
     */
    SMS.prototype.credit_sub_account = function(sub_account_email, sms_amount, sms_type, callback) {
        var self = this;
        var domain = DOMAIN;
        var path = PATH_CREDIT_SUB_ACCOUNT_TOKEN;
        var port = PORT;
        ...
        this._Request(domain, path, port, data, function(error, xml_return){
            if (error == true) {
                ...
                self._Request(domain, path, port, data, function(error, xml_return){
                    callback(error, xml_return);
                });
            }
        });
     ...
     * @param {function} callback - callback function that takes (error, response) parameters
     */
    SMS.prototype.get_balance = function(callback) {
        var domain = DOMAIN;
        var path = PATH_BALANCE;
        var port = PORT;
     ...
     * Sets SMS send mode
     *
     * @param {int} sms_mode - octopush.constants.INSTANTANE or octopush.constants.DIFFERE
     */
    SMS.prototype.set_sms_mode = function(sms_mode) {
        this.config.sms_mode = sms_mode;
    };
     ...
     * @param {int} i - minute
     */
    SMS.prototype.set_date = function(y, m, d, h, i) {
        this.config.sms_y = y;
        this.config.sms_m = d;
        this.config.sms_d = m;
        this.config.sms_h = h;
        ...
        this.config.sending_date = Math.floor(new Date(y, (m - 1), d, h, i).getTime() / 1000);
    };
